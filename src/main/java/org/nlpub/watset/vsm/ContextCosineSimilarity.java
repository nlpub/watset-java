...
public class ContextCosineSimilarity<V> implements ContextSimilarity<V> {
    ...
    public Number apply(Map<V, Number> bag1, Map<V, Number> bag2) {
        ...
        final Map<V, Number> vec1 = Vectors.transform(bag1, union);
        final Map<V, Number> vec2 = Vectors.transform(bag2, union);

        final double denominator = Vectors.norm(vec1) * Vectors.norm(vec2);

        if (denominator == 0.) return 0d;

        return Vectors.dot(vec1, vec2) / denominator;
    }
}
